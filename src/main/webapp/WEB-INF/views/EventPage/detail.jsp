    ...
    <section class="py-5 text-center container">
        <div class="row py-lg-5">
            <div class="col-lg-10 col-md-8 mx-auto">
                ...
                <p class="lead text-muted" style="font-family: 'IM_Hyemin-Bold'; font-size: 24px;"><br>이벤트 상세 페이지<br>
                </p>
                <div align="center"
                    ...
                    <c:choose>
                        <c:when test="${sessionScope.loginMemberId eq 'admin'}">
                            <div>
                                <input type="button" class="btn btn-dark" onclick="eventDelete(${event.id})"
                                       value="이벤트 삭제">
                            </div>
                        </c:when>
                    </c:choose>
                    ...
                    <div>
                        <span>현 재고 : </span>
                        <span>${event.eventStock}</span>
                    </div>
                    <br>
                    <div>
                        <span>이벤트 설명</span><br>
                        <span>${event.eventContents}</span>
                    </div>
                    <div>
...
<script>
    ...
    const productPay = () => {
        ...
        console.log("수량 " + eventQuantity);
        console.log("재고 " + eventStock);
        console.log("재고-수량" + eventQuantityToBuy);
        if (eventStock == 0) {
            alert('품절');
        } else if (eventQuantityToBuy < 0) {
            alert('재고부족');
        } else {
            paying()
        }
